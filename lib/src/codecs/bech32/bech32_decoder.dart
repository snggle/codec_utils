...
class Bech32Decoder extends Converter<String, Bech32Pair> {
  ...
  Bech32Pair convert(String input, [int maxInputLength = Bech32Validation.maxInputLength]) {
    ...
    int checksumLength = input.length - separatorPosition - 1 - maxChecksumLength;

    if (Bech32Validation().isChecksumTooShort(separatorPosition, input)) {
      throw Exception('Checksum is to short: ${input}');
    }

    input.toLowerCase();

    String hrp = input.substring(0, separatorPosition);
    String data = input.substring(separatorPosition + 1, input.length - checksumLength);
    String checksum = input.substring(input.length - checksumLength);
